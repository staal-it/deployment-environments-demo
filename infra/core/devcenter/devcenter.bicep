...
resource attachednetworks 'Microsoft.DevCenter/devcenters/attachednetworks@2023-10-01-preview' = {
  name: '${name}-network'
  parent: devcenter
  properties: {
...
module devCenterProject 'project.bicep' = [for project in config.projects: {
  ...
  params: {
    ...
    networkConnectionName: attachednetworks.name
  }
}]
...
module devBoxDefinition 'devbox-definition.bicep' = {
  name: '${deployment().name}-devbox-definition'
  params: {
    devCenterName: devcenter.name
    location: location
    name: 'devbox-def'
